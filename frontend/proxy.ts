...
function createRedirectUrl(basePath: string, request: NextRequest, redirectTo?: string): URL {
  ...
  console.error(`[Proxy] Creating redirect URL: ${url.toString()}`);
  if (redirectTo) {
...
export async function proxy(request: NextRequest) {
  ...
  console.error(`[Proxy] ${pathname} - Token: ${token ? 'exists' : 'missing'}`);

  // Allow root route to display landing page (handled client-side)
  if (pathname === '/') {
    console.error(`[Proxy] Root route - Allowing access to landing page`);
    return NextResponse.next();
  }
  ...
  if (pathname.startsWith('/login') || pathname.startsWith('/register')) {
    if (token) {
      ...
      const safeRedirect = redirectTo.startsWith('/') && !redirectTo.startsWith('//') 
        ? redirectTo 
        : '/dashboard';
      console.error(`[Proxy] Auth route - Redirecting to ${safeRedirect} (token exists)`);
      return NextResponse.redirect(new URL(safeRedirect, request.url));
    }
    // Allow access to auth pages
    console.error(`[Proxy] Auth route - Allowing access (no token)`);
    return NextResponse.next();
  }
  ...
  if (
    ...
    if (!token) {
      ...
      console.error(`[Proxy] Protected route ${pathname} - Redirecting to /login (no token)`);
    ...
    console.error(`[Proxy] Protected route ${pathname} - Allowing access (token exists)`);
    return NextResponse.next();
  }
  ...
  if (pathname.startsWith('/admin')) {
    if (!token) {
      // No token, redirect to login with redirect parameter
      console.error(`[Proxy] Admin route ${pathname} - Redirecting to /login (no token)`);
      return NextResponse.redirect(createRedirectUrl('/login', request, pathname));
    }
    // Token exists, allow access (role check happens on client side)
    console.error(`[Proxy] Admin route ${pathname} - Allowing access (token exists)`);
    return NextResponse.next();
  }

  // Allow other routes (API routes, static files, etc.)
  console.error(`[Proxy] ${pathname} - Allowing access (not matched)`);
  return NextResponse.next();
}
